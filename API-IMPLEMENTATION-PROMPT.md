...
The API should:
- Implement the endpoint specified in [Hello Endpoint Specification](./docs/api/specs/hello-endpoint.md)
- Follow all design principles in [API Design Specification](./docs/api/specs/api-design.md)
- Handle errors according to [Error Handling Specification](./docs/api/specs/error-handling.md)
- Use the technology stack defined in [Technology Choices Specification](./docs/api/specs/technology-choices.md)
...
- Be structured to scale following the workflow in [Adding Endpoints Guide](./docs/api/guides/adding-endpoints.md)
...
1. Review [Getting Started Guide Design Specification](./docs/api/specs/getting-started-design.md) for the abstract structure
2. Generate `/docs/api/guides/getting-started.md` with:
...
1. Review [Restoration Prompt Design Specification](./docs/api/specs/restore-prompt-design.md) for the abstract structure
2. Generate `/API-RESTORATION-PROMPT.md` with:
   ...
   - Safe deletion commands for all generated artifacts (/src, /tests, global.json, hello-world-api.sln, .gitignore, docs/api/guides/getting-started.md, API-CLAUDE.md)
   - Confirmation that all generated files are removed
   - Post-restoration verification checklist
   - **CRITICAL**: Self-deletion instruction as final step (remove API-RESTORATION-PROMPT.md itself)
...
After API-RESTORATION-PROMPT.md is generated and all code has been compiled and tested successfully:
...
3. It must contain:
   ...
   - **Links to Detailed Documentation**: All `/docs/api/specs/`, `/docs/api/guides/`, and `/docs/api/adr/` files with descriptions
...
**Note**: `API-CLAUDE.md` is a generated artefact. It will be removed by `API-RESTORATION-PROMPT.md` if the project is restored to specification-only state, and regenerated when `API-IMPLEMENTATION-PROMPT.md` is executed again.
