...
**Why**: To serve as the canonical working example produced by the SCRIBE methodology. This project was generated entirely from markdown specifications by executing `API-IMPLEMENTATION-PROMPT.md` — no code was written manually.
...
**Relationship to SCRIBE**: This project is a *generated artefact*. The methodology is documented in `CLAUDE.md` and `README.md`. This file (`API-CLAUDE.md`) was generated by `API-IMPLEMENTATION-PROMPT.md` Part 5 and is specific to this generated project.
...
### Quick Context
This project follows specification-first development. All requirements are documented in `/docs/` before any code exists. Code is generated from specs — never the other way around. If you need to understand *why* something is built a certain way, read the spec files in `/docs/api/specs/` first.
...
- **`src/api/HelloWorldApi.csproj`** — Targets `net8.0`; references `Microsoft.AspNetCore.OpenApi` and `Swashbuckle.AspNetCore`.
- **`tests/HelloWorldApi.Tests.csproj`** — xUnit test project using `WebApplicationFactory<Program>`.

### Documentation Navigation
- [Hello Endpoint Spec](./docs/api/specs/hello-endpoint.md) — Full contract for `POST /test/hello`
- [API Design Principles](./docs/api/specs/api-design.md) — Endpoint design standards and naming
- [Error Handling Spec](./docs/api/specs/error-handling.md) — Error envelope format and HTTP codes
- [Technology Choices](./docs/api/specs/technology-choices.md) — Why ASP.NET Core, .NET 8, xUnit
- [Getting Started Guide](./docs/api/guides/getting-started.md) — Human-oriented setup guide
- [Adding Endpoints Guide](./docs/api/guides/adding-endpoints.md) — Workflow for new endpoints
- [ADR-001: Minimal APIs](./docs/api/adr/adr-001-minimal-apis.md) — Why Minimal APIs over MVC
...
- Validation error: HTTP 400 with `code = "INVALID_REQUEST"` (see [Error Handling Spec](./docs/api/specs/error-handling.md))
...
Create `/docs/api/specs/{endpoint-name}.md` following the pattern in `hello-endpoint.md`:
...
Add an entry to [API Design — Endpoint Index](./docs/api/specs/api-design.md) with status: `Active`, `Planned`, or `Deprecated`.
...
If the endpoint requires special setup or configuration, update `/docs/api/guides/` accordingly.
...
### Specifications
- [Hello Endpoint Contract](./docs/api/specs/hello-endpoint.md) — `POST /test/hello` full specification
- [API Design Principles](./docs/api/specs/api-design.md) — Endpoint design standards and endpoint index
- [Error Handling Specification](./docs/api/specs/error-handling.md) — Error envelope format, HTTP codes, error code registry
- [Technology Choices](./docs/api/specs/technology-choices.md) — Framework, runtime, and testing framework decisions

### Guides
- [Getting Started](./docs/api/guides/getting-started.md) — Full setup guide for humans (prerequisites, clone, build, run, test)
- [Adding Endpoints](./docs/api/guides/adding-endpoints.md) — Step-by-step workflow for adding new endpoints

### Architecture Decisions
- [ADR-001: Minimal APIs](./docs/api/adr/adr-001-minimal-apis.md) — Why Minimal APIs over MVC Controller pattern

---

## Document Metadata

- **Version**: 1.0 (Phase 1 — generated by API-IMPLEMENTATION-PROMPT.md Part 5)
- **Last Updated**: 2026-02-18
- **Audience**: Claude Code agents working on the generated Hello World API project
- **Generated By**: `API-IMPLEMENTATION-PROMPT.md` — regeneratable; removed by `API-RESTORATION-PROMPT.md`
- **Methodology Guidance**: See [`CLAUDE.md`](./CLAUDE.md) for the repository-level SCRIBE methodology context
- **Human Quick Start**: See [`docs/api/guides/getting-started.md`](./docs/api/guides/getting-started.md)
